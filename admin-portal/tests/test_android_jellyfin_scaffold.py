...
def test_jellyfin_browse_and_details_fragments_scaffold_home_section_and_playback_handoff():
    ...
    assert 'class JellyfinBrowseFragment : Fragment()' in browse
    assert 'Jellyfin' in browse
    assert 'Movies' in browse
    assert 'TV Shows' in browse
    ...
    assert 'ScrollView(requireContext())' in details
    assert 'item.overview?.takeIf' in details
    assert 'text = "Play"' in details
